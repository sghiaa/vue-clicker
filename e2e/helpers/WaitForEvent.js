...
const multiWait = async (events, timeoutSecondsParam) => {
    ...
    for (let i = 0; i < eventLabels.length; i++) {
      ...
      if (eventFunction === "NOTHING") {
        ...
        if (await eventFunction()) {
          ...
          foundEvent = label;
          logMultiWaitDebug(
            `  - foundEvent has now been set to [${foundEvent}]`
          );
          break;
        } else {
  ...
  if (foundEvent === null) {
    logMultiWaitDebug(
      `- Throwing error! After [${timer.tryCount}] polling iterations over [${timeoutSeconds}] seconds, none of the following events occurred: [${eventLabels}].`
    );
    throw new Error(
      `After [${timer.tryCount}] polling iterations over [${timeoutSeconds}] seconds, none of the following events occurred: [${eventLabels}].`
    );
  } else {
...
const waitFor = async (eventFunction, options = {}) => {
  ...
  logWaitForDebug(
    `waitFor called on event [${description}] with a timeout of [${timeoutSeconds}] seconds.`
  );
